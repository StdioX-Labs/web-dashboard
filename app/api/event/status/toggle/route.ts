  ...
  try {
    ...
    if (!eventId || !userId) {
      return NextResponse.json(
        { status: false, message: 'eventId and userId are required' },
        { status: 400 }
      )
    }
    ...
    const response = await fetch(`${BASE_URL}/event/status/toggle?eventId=${eventId}&userId=${userId}`, {
      method: 'POST',
      headers,
    ...
    return NextResponse.json(
      { status: false, message: error instanceof Error ? error.message : 'Failed to toggle event status' },
      { status: 500 }
    )
  }
}
