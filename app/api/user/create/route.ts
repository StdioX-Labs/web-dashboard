...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.fullName || !body.mobileNumber || !body.emailAddress || !body.password || !body.roles || !body.company?.id) {
    ...
    const backendUrl = `${BASE_URL}/user/create`
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: data.message || "Failed to create user", status: false },
